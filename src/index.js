...
const P = useTheme(({children, style}) => (
  <div style={{
    fontWeight: 300,
    lineHeight: 1.5,
    ...style
...
function ActiveDecorator(Component, activeProps) {
  const Option = useTheme((props, context) => (
    <Component {...(props.active ? {...props, ...activeProps(props, context)} : props)} />
  ));
  return Option;
}
...
const SpacingExample = useTheme(
  (props, {g, ms}) => (
    <div style={{
      display: 'flex',
      backgroundColor: g.base(0.1)
    }}>
      <SpacedFlexbox spacing={ms.spacing(2)} style={{justifyContent: 'center'}}>
        <Box />
        <Box />
        <Box />
        <Box />
...
const ModularScaleExample = useTheme(
  (props, {g, ms}) => (
    <div style={{backgroundColor: g.base(0.1), padding: ms.spacing(10)}}>
      <a style={{
        fontSize: ms.tx(0),
        padding: ms.spacing(2),
        marginRight: ms.spacing(1),
        borderWidth: ms.border(2),
        borderStyle: 'solid'
      }}>
        Better Way
      </a>
      ...
      <Content style={{
        ...
        <VGroup spacing={ms.spacing(5)}>
          ...
          <P>
            ...
            <List style={{marginTop: ms.spacing(2)}}>
          ...
          <div style={{
            ...
            <VGroup spacing={ms.spacing(5)} style={{justifyContent: 'center'}}>
...
const Option = ActiveDecorator(ListItem,
  (props, {g}) => ({
    style: {
      color: g.primary(1),
...
const App = React.createClass({
  ...
  render () {
    return (
      <ThemeContext g={Gradient.create('yellow', 'blue')}>
        <Fill>
          <Nav />
